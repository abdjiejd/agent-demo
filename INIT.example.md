...
### 2. 生成后端 .env
读取 backend/.env.example，将用户填写的配置写入 backend/.env，格式：
```
PROJECT_NAME=<用户值>
SERVER_PORT=<用户值>
FRONTEND_PORT=<用户值>
...
VITE_SERVER_PORT=<用户填的FRONTEND_PORT值>
VITE_BACKEND_URL=http://localhost:<用户填的SERVER_PORT值>/api
```
...
venv\Scripts\uvicorn app.main:app --host 0.0.0.0 --port <用户填的SERVER_PORT值> --reload
```
...
### 6. 验证
后端: http://localhost:<用户填的SERVER_PORT值>/docs
前端: http://localhost:<用户填的FRONTEND_PORT值>
